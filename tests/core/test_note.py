...
class TestNoteCreation:
    def test_create_note(self) -> None:
        ...
        assert n.duration == Duration.from_cn("q")
        assert n.dynamic == "mf"
    ...
    def test_note_no_articulations(self) -> None:
        n = Note(Pitch("c", "n", 4), Duration.from_cn("q"), "mf", ())
...
class TestNoteImmutability:
    ...
    def test_identical_hash_equally(self) -> None:
        n1 = Note(Pitch("c", "n", 4), Duration.from_cn("q"), "mf", ("stacc",))
        n2 = Note(Pitch("c", "n", 4), Duration.from_cn("q"), "mf", ("stacc",))
        assert n1 == n2
...
class TestRestCreation:
    def test_create_rest(self) -> None:
        r = Rest(Duration.from_cn("q"))
        assert r.duration == Duration.from_cn("q")
...
class TestRestImmutability:
    def test_frozen(self) -> None:
        r = Rest(Duration.from_cn("q"))
        with pytest.raises(AttributeError):
            r.duration = Duration.from_cn("h")  # type: ignore[misc]
