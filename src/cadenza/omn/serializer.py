"""OMN serializer: converts Phrase to compact OMN notation string."""
...
def to_omn(phrase: Phrase) -> str:
    """Serialize a Phrase to compact OMN notation.
    ...
    for event in phrase:
        if isinstance(event, Rest):
            parts.append(_rest_to_omn(event.duration))
        ...
        if isinstance(event, Note):
            ...
            # OMN cannot "unset" a dynamic, so only update prev when a
            ...
            # Empty articulations mean "keep previous" in OMN, so only
...
def _duration_to_omn(d: Duration) -> str:
    """Convert a Duration to its OMN string representation."""
    result = ""
...
def _pitch_to_omn(p: Pitch) -> str:
    """Convert a Pitch to its OMN string representation."""
    acc = "" if p.accidental == "n" else p.accidental
    return f"{p.step}{acc}{p.octave}"


def _rest_to_omn(d: Duration) -> str:
    """Convert a rest Duration to its OMN string representation."""
    return "-" + _duration_to_omn(d)
